...
export default function AdBanner() {
  ...
  // Установить фиксированную высоту контейнера после загрузки
  useEffect(() => {
    if (!bannerRef.current || loaded) return;
    
    // Устанавливаем высоту только один раз после первой загрузки
    if (bannerRef.current.offsetHeight > 0) {
  ...
  }, [isLoading, loaded, ads]);
  
  // Запасной баннер для всех состояний ошибки/загрузки
  const fallbackBanner = (
    <div className="banner-content px-6 py-8 sm:p-10 sm:flex sm:items-center">
  ...
  if (isLoading) {
    return (
      <div 
        ref={bannerRef}
        className="banner-container bg-gradient-to-r from-blue-600/40 to-indigo-700/40 rounded-xl shadow-md overflow-hidden h-64"
      >
        <BannerSkeleton />
      </div>
    );
  ...
  if (error || !ads || ads.length === 0) {
    return (
      <div 
        ref={bannerRef}
        className="banner-container bg-gradient-to-r from-blue-600 to-indigo-700 rounded-xl shadow-md overflow-hidden h-64"
      >
        {fallbackBanner}
      </div>
    );
  ...
  return (
    <div 
      ref={bannerRef}
      className={`banner-container bg-gradient-to-r ${currentAd.bgColor || "from-blue-600 to-indigo-700"} rounded-xl shadow-md overflow-hidden relative text-white h-64`}
      style={bgStyle}
    >
      ...
      <div className="banner-content px-6 py-8 sm:p-10 sm:flex sm:items-center h-full">
        <div className="sm:flex-1">
          <h3 className="text-xl font-extrabold text-white sm:text-2xl">
            {currentAd.title}
          </h3>
...
function BannerSkeleton() {
  return (
    <div className="banner-content animate-pulse px-6 py-8 sm:p-10 sm:flex sm:items-center">
      <div className="sm:flex-1">
