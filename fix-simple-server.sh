...
# ==========================================
# СКРИПТ ДЛЯ ИСПРАВЛЕНИЯ СЕРВЕРА С ES MODULES
# ==========================================
# Версия: 1.0.0
# Дата: 01.05.2025
...
if [ "$(id -u)" != "0" ]; then
   ...
   error "Пожалуйста, используйте: sudo ./fix-simple-server.sh"
   exit 1
fi
...
# Копирование скрипта ES-модуля сервера
log "Копирование сервера ES-модуля..."
cp simple-server.mjs $PROJECT_DIR/
success "Сервер скопирован"
...
pm2 save
success "Сервер запущен через PM2"

# Проверка статуса через 5 секунд (для уверенности)
log "Проверка статуса сервера через 5 секунд..."
sleep 5
...
echo "=================================================================="
echo "        НАСТРОЙКА ПРОСТОГО ES-МОДУЛЯ СЕРВЕРА ЗАВЕРШЕНА!"
echo "=================================================================="
echo ""
