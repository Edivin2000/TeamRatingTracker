...
GIT_REPO_URL="https://github.com/Edivin2000/TeamRatingTracker.git"
...
# Останавливаем процессы, которые могут мешать установке
log "Освобождение портов и остановка конфликтующих сервисов..."
...
# Проверка и остановка процессов, использующих нужные порты
if lsof -i:80 >/dev/null 2>&1; then
  warning "Порт 80 занят. Попытка освободить..."
  pkill -f "nginx" || true
  fuser -k 80/tcp >/dev/null 2>&1 || true
fi

if lsof -i:$SERVER_PORT >/dev/null 2>&1; then
  warning "Порт $SERVER_PORT занят. Попытка освободить..."
  fuser -k $SERVER_PORT/tcp >/dev/null 2>&1 || true
fi

# Переходим в корневую директорию для предотвращения ошибок с директориями
cd /
...
apt install -y curl git nginx postgresql postgresql-contrib build-essential unzip gpg ca-certificates apt-transport-https

# Удаление всех версий Node.js и npm перед установкой
log "Удаление существующих версий Node.js и npm..."
...
# Создаем временный файл для установки Node.js
cd /tmp
...
if command -v node &> /dev/null; then
  NODE_VERSION=$(node -v)
  log "Успешно установлен Node.js $NODE_VERSION"
else
...
if command -v npm &> /dev/null; then
  NPM_VERSION=$(npm -v)
  log "Успешно установлен npm $NPM_VERSION"
else
  error "Не удалось найти npm. Возможно, проблема с установкой Node.js."
fi
...
if curl -s "http://localhost:$SERVER_PORT/api/health" | grep -q "ok"; then
  ...
  warning "Приложение не отвечает на /api/health. Попробуем простой запрос..."
  if curl -s "http://localhost:$SERVER_PORT/api" &>/dev/null; then
    log "Приложение работает и отвечает на запросы к /api!"
  else
