...
# Скрипт для ручного запуска сервера ATOM-GAME
# Используйте эту команду для запуска: sudo bash manual-run.sh

# Переменные окружения
export PORT=5000
export DATABASE_URL="postgresql://atomgame:Atom%26Game%232025!@localhost:5432/atomgame"
export SESSION_SECRET="AtomGameSecretSession2025!"
export UPLOADS_DIR="/var/www/atomgameblk/uploads"
...
# Установка необходимых пакетов для нового сервера
echo "Установка необходимых пакетов..."
npm install express pg express-session connect-pg-simple

# Копирование create-server.js в директорию, если файл существует в текущей директории
if [ -f "create-server.js" ]; then
  cp create-server.js /var/www/atomgameblk/simple-server.js
  echo "Файл create-server.js скопирован в директорию проекта."
else
  # Создаем файл простого сервера прямо в директории проекта
  cat > /var/www/atomgameblk/simple-server.js << 'EOL'
// Простой сервер для ATOM-GAME
...
EOL
  echo "Файл простого сервера создан."
fi

# Запуск сервера через Node.js напрямую
echo "Запуск сервера через Node.js напрямую..."
cd /var/www/atomgameblk
node simple-server.js > /var/log/atomgame-server.log 2>&1 &
SIMPLE_SERVER_PID=$!

# Сохранение PID в файл
echo $SIMPLE_SERVER_PID > /var/www/atomgameblk/server.pid
echo "Сервер запущен с PID: $SIMPLE_SERVER_PID"
echo "Логи доступны в файле: /var/log/atomgame-server.log"

# Инструкции по остановке
echo ""
echo "СЕРВЕР УСПЕШНО ЗАПУЩЕН И ДОСТУПЕН ПО АДРЕСУ: http://localhost:5000"
echo "Для входа в админку используйте:"
echo "  Логин: admin"
echo "  Пароль: Atom&Game#2025!"
echo ""
echo "Чтобы остановить сервер, выполните команду:"
echo "kill \$(cat /var/www/atomgameblk/server.pid)"